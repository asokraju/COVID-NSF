...
def worker(actor, critic, num_of_envs = 4):
    ...
    #     samples_rnn_partial = partial(samples_rnn, actor = actor, critic=critic)
    #     future_to_samples = {executor.submit(samples_rnn_partial, env): env for env in env_list}
    ...
    actor_list = [clone_model(actor) for _ in range(num_of_envs)]
    critic_list = [clone_model(critic) for _ in range(num_of_envs)]
    partial_sample = partial(samples, actor=actor, critic=critic)
    # results = pool.map(samples, zip(env_list,actor_list,critic_list))
    results = pool.map(partial_sample, env_list)
    pool.close()
    pool.join()
    States, Actions, Rewards = [], [], []
    States, Actions, Rewards = [], [], []
    for S, A, R, score in results:
